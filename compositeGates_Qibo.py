...
def auxiliary_gate(circuit, qubit):
...
def restore_gate(circuit, qubit):
    circuit.add(gates.X(qubit))
    circuit.add(gates.Z(qubit))
    circuit.add(gates.X(qubit))
    circuit.add(gates.Z(qubit))
...
def encapsulate_original_gate(gate, num_qubits):
    sub_circuit = Circuit(num_qubits, density_matrix=True)
    if isinstance(gate, gates.Unitary):
        sub_circuit.add(gate)
    elif isinstance(gate, gates.Gate):
        sub_circuit.add(gate.__class__(*gate.qubits, **gate.init_kwargs))
    return sub_circuit

def apply_auxiliary_gates(circuit, qubits):
    for qubit in qubits:
        auxiliary_gate(circuit, qubit)

def apply_restore_gates(circuit, qubits):
    for qubit in qubits:
        restore_gate(circuit, qubit)

def obfuscate_circuit(circuit, obfuscate=False):
    if obfuscate:
        qubits = range(circuit.nqubits)
        apply_auxiliary_gates(circuit, qubits)
        apply_restore_gates(circuit, qubits)
    return circuit
...
def insert_obfuscation(circuit, encapsulate_probability=0.5):
    ...
    for gate in circuit.queue:
        if isinstance(gate, gates.M):
            ...
            if not obfuscation_done:
                obfuscate_circuit(new_circuit, obfuscate=True)
                obfuscation_done = True

            if random.random() < encapsulate_probability:
                encapsulated_circuit = encapsulate_original_gate(gate, circuit.nqubits)
                for enc_gate in encapsulated_circuit.queue:
                    new_circuit.add(enc_gate)
            else:
    ...
    for gate in measurement_instructions:
        new_circuit.add(gates.M(*gate.qubits, collapse=False))
...
def cg_obfuscate_and_execute_qibo(input_qasm):
...
if __name__ == "__main__":
    ...
    results_2 = cg_obfuscate_and_execute_qibo(test_qasm)
