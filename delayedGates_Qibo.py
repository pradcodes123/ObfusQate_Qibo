...
def apply_complex_obfuscation(circuit, qubits):
    identity_sequences = [
        ["y", "s", "y"],
        ["h", "s", "h", "s", "h"],
        ["x", "h", "z", "h", "x"],
            ...
            for gate_name in sequence:
                ...
                if gate_name in ["cx", "swap"]:
                    for i in range(0, len(qubits) - 1, 2):
                        circuit.add(gate_class(qubits[i], qubits[i + 1]))
                else:
                    circuit.add(gate_class(qubit))

def obfuscate_circuit(circuit, qubits, obfuscate=False):
    if obfuscate:
        apply_complex_obfuscation(circuit, qubits)
    return circuit
...
def insert_obfuscation(circuit):
    ...
    obfuscate_circuit(new_circuit, qubits, obfuscate=True)

    for gate in circuit.queue:
        if isinstance(gate, gates.M):
            measurement_instructions.append(gate)
        else:
            new_circuit.add(gate.__class__(*gate.qubits, **gate.init_kwargs))
...
def dg_obfuscate_and_execute_qibo(input_qasm):
...
if __name__ == "__main__":
    ...
    results_2 = dg_obfuscate_and_execute_qibo(test_qasm)
